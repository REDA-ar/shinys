...
ui <- fluidPage(
  titlePanel("TCL con histogramas"),
  
  sidebarLayout(
    sidebarPanel(
      #helpText("Histograma de normales"),
     
      numericInput("ene",h3("Cantidad de valores para  calcular el promedio"), value=1),
      
    selectInput( "distribucion", h3("Elija la distribucion"), 
                             choices = list("Uniforme" = "uniforme", 
                                            "Exponencial" = "exponencial","Normal" = "normal"
                                            ), selected = 1),
    
    radioButtons(
      "estandiarizar",
      "¿Estandarizamos los promedios?",
      c("NO","SI"),
      inline = TRUE,
      selected = "NO"
    ),
...
#     Output: Histogram ----
     plotOutput(outputId = "histPromedios")
...
#  input: aca se guardan todas las cosas que fuimos 
  ...
  Nrep<-10000
    ...
    output$histPromedios <- renderPlot({
      n<-input$ene
    ...
    { 
      mu<-70
      sigma<-3
      todos<-matrix(rnorm(n*Nrep,mu,sigma),Nrep,n)
      promedios<-apply(todos,1, mean)
      estandarizados<-sqrt(n)*(promedios-mu)/sigma
    ...
    { 
      ...
      mu<-1/lambda
      sigma<-1/lambda
      todos<-matrix(rexp(n*Nrep,lambda),Nrep,n)
      promedios<-apply(todos,1, mean)
      estandarizados<-sqrt(n)*(promedios-mu)/sigma
      } 
    ...
    {  
      ...
      mu<-(a+b)/2
      sigma<-sqrt((b-a)^ 2/12)
      todos<-matrix(runif(n*Nrep,a,b),Nrep,n)
      promedios<-apply(todos,1, mean)
      estandarizados<-sqrt(n)*(promedios-mu)/sigma
    }
      
     if(input$estandiarizar=="SI")  
     {salida <- hist(estandarizados,freq=FALSE,xlab="promedios estandarizados", 
           col = "#75AADB", border = "white",main = "Histograma")
     }
    
      if(input$estandiarizar=="NO")  {
    salida  <-   hist(promedios,freq=FALSE,xlab="promedios sin estandarizar", 
         col = "#75AADB", border = "white",main = "Histograma")}
